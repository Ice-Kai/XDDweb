...
- 旧 PHP 网站不再继续迭代，文件保留在服务器上作为历史后备，不主动删除。
- 正式主站以后以新 Astro 站为准，`xuedda.com` 直接访问新站，不再做“新旧站入口选择页”。
- 新站直接复用旧站真实数据库：`www_xuedda_com`。
- 旧会员、旧栏目、旧下载记录继续留在原库内，新站逐步接管登录、下载、后台上传与内容管理。
- 上传图片不再从本地重复整体传服务器。线上 `/uploads` 已指向旧站上传目录：
  - `/www/wwwroot/xuedda-next/dist/client/uploads -> /www/wwwroot/www.xuedda.com/uploads`
- 后续如果图片缺失，优先检查数据库里的封面路径是否真实存在于旧站 `/uploads`，不要先做本地全量上传。
- 旧 PHP 目录保留但视为冻结资产，除紧急回滚外不要再往旧站写新功能。
